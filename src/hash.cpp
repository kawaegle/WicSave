...
void MD5::processBuffer()
{
    ...
    // number of bits
    size_t padded_length = m_bufferSize * 8;

    // plus one bit set to 1 (always appended)
    padded_length++;

    // number of bits must be (numBits % 512) = 448
    size_t lower11_bits = padded_length & 511;
    if (lower11_bits <= 448)
        padded_length += 448 - lower11_bits;
    else
        padded_length += 512 + 448 - lower11_bits;
    ...
    for (; i < padded_length; i++) extra[i - BlockSize] = 0;

    // add message length in bits as 64 bit number
    uint64_t msg_bits = 8 * (m_numBytes + m_bufferSize);
    // find right position
...
void MD5::getHash(unsigned char buffer[MD5::HashBytes])
{
    ...
    for (int i = 0; i < HashValues; i++) old_hash[i] = m_hash[i];
